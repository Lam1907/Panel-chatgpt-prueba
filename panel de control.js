 ...
 * control-panel.js
 *
 * Version limpia: panel sincronizado con el modulo publico de votacion.
 *
 ...
 *   - El modulo Vote escribe votos individuales en /votaciones
 *   - El panel agrega esos votos en memoria antes de consolidar
...
let state = {
  ...
  // Votos individuales escritos por el modulo publico en /votaciones/{pushId}.
  votaciones: {},

  // Agregado interno construido desde /votaciones.
  // Forma: { pareja_1: { votos: { participant_1__participant_7: 3 } } }
  voteAggregate: {},

  // Parejas consolidadas escritas en /results/pairs.
  pairs: {},
...
function setupListeners() {
  ...
  if (!_unsubs.state) {
    _unsubs.state = onValue(ref(db, 'state'), snap => {
      ...
      state.fb = {
        ...
        currentRound: parseInt(s.currentRound || 1, 10),
        timerEnd:     s.timerEnd    || 0,
        voteSession:  s.voteSession || '',
      ...
      if (state.ready.votaciones) {
        state.voteAggregate = aggregateVotaciones(state.votaciones, state.fb.currentRound);
        UI.displayVotes.textContent = String(countVotaciones(state.votaciones, state.fb.currentRound)).padStart(3, '0');
      }
  ...
  // /votaciones — ruta real donde el modulo publico escribe cada voto
  if (!_unsubs.votaciones) {
    _unsubs.votaciones = onValue(ref(db, 'votaciones'), snap => {
      state.votaciones    = snap.val() || {};
      state.voteAggregate = aggregateVotaciones(state.votaciones, state.fb.currentRound);
      state.ready.votaciones = true;

      const total = countVotaciones(state.votaciones, state.fb.currentRound);
      UI.displayVotes.textContent = String(total).padStart(3, '0');

      if (state.ready.state && state.fb.totalVotes !== total) {
        update(ref(db, 'state'), { totalVotes: total }).catch(err => {
          console.warn('[CP] No se pudo sincronizar totalVotes:', err.message);
        });
      }

      console.log('[CP] /votaciones ->', total, 'votos ronda', state.fb.currentRound);
      console.log('[CP] agregado ->', JSON.stringify(state.voteAggregate));
...
/**
 * countVotaciones — cuenta votos individuales reales escritos por Vote en /votaciones.
 * Filtra por la ronda activa para no mezclar rondas antiguas.
 */
function countVotaciones(votaciones, round) {
  return Object.values(votaciones || {}).filter(v => isVoteForRound(v, round)).length;
}

function isVoteForRound(vote, round) {
  if (!vote || typeof vote !== 'object') return false;
  return parseInt(vote.round || 1, 10) === parseInt(round || 1, 10);
}

function participantNumberFromId(id) {
  const m = String(id || '').match(/\d+/);
  return m ? parseInt(m[0], 10) : null;
}

function normalizedComboKey(aId, bId) {
...
/**
 * aggregateVotaciones — adapta /votaciones al formato que necesita calcPairs().
 * Entrada real:
 *   /votaciones/{pushId}/parejas/pareja_1/participantes: [{id, nombre}, {id, nombre}]
 * Salida interna:
 *   { pareja_1: { votos: { participant_1__participant_7: 2 } } }
 */
function aggregateVotaciones(votaciones, round) {
  const aggregate = {};

  for (let p = 1; p <= TOTAL_PAIRS; p++) {
    aggregate[`pareja_${p}`] = { votos: {} };
  }

  Object.values(votaciones || {}).forEach(vote => {
    if (!isVoteForRound(vote, round)) return;

    const parejas = vote.parejas || {};
    for (let p = 1; p <= TOTAL_PAIRS; p++) {
      const voteKey = `pareja_${p}`;
      const slot = parejas[voteKey];
      const pts = Array.isArray(slot?.participantes) ? slot.participantes : [];
      if (pts.length < 2) continue;

      const idA = pts[0]?.id;
      const idB = pts[1]?.id;
      const combo = normalizedComboKey(idA, idB);
      if (!combo) continue;

      const votos = aggregate[voteKey].votos;
      votos[combo] = (votos[combo] || 0) + 1;
    }
  });
...
$('btnStartVoting').addEventListener('click', async () => {
  ...
  await withLoading(async () => {
    ...
    console.log('[CP] INICIAR VOTACION -> timerEnd =', end, 'session =', session);
...
$('btnConsolidate').addEventListener('click', async () => {
  ...
  await withLoading(async () => {
    const pairs = calcPairs(state.voteAggregate);
 ...
 * 1. Para cada cuadro (pareja_1..pareja_5), ordena combinaciones por votos.
 * 2. Elige la combinacion mas votada que no repita participantes ya usados.
 * 3. Rellena cuadros incompletos con participantes sobrantes para que nadie quede fuera.
 */
function calcPairs(aggregate) {
  const totalVotes = countVotaciones(state.votaciones, state.fb.currentRound);
  if (totalVotes <= 0) return null;

  console.log('[CP] calcPairs — agregado:', JSON.stringify(aggregate));
  ...
  for (let p = 1; p <= TOTAL_PAIRS; p++) {
    ...
    ranked[pk] = Object.entries(votos)
      .map(([combo, count]) => {
        const [idA, idB] = combo.split('__');
        const a = participantNumberFromId(idA);
        const b = participantNumberFromId(idB);
        return { a, b, v: Number(count) || 0, combo };
      })
      .filter(x => Number.isInteger(x.a) && Number.isInteger(x.b) && x.a !== x.b)
      .sort((x, y) => y.v - x.v);
  ...
  for (let p = 1; p <= TOTAL_PAIRS; p++) {
    ...
    for (const { a, b, v } of ranked[pk]) {
      if (v <= 0) continue;
      if (used.has(a) || used.has(b)) continue;

      used.add(a);
      used.add(b);
      result[pk] = { participants: [a, b], eliminated: false, votes: v };
      console.log(`[CP] ${pk} -> [${a},${b}] por votos: ${v}`);
      assigned = true;
      break;
    }
  ...
  const available = Array.from({ length: TOTAL_PARTICIPANTS }, (_, i) => i + 1)
    .filter(n => !used.has(n));
  ...
  for (let p = 1; p <= TOTAL_PAIRS; p++) {
    ...
    const a = available.shift() ?? null;
    const b = available.shift() ?? null;
    result[pk] = { participants: [a, b], eliminated: false, votes: 0, filled: true };
    console.log(`[CP] ${pk} -> [${a},${b}] relleno por sobrantes`);
  }
...
$('btnFinalizeRound').addEventListener('click', async () => {
  ...
  await withLoading(async () => {
    ...
    await set(ref(db, `history/round_${rnd}`), {
      ...
      voteAggregate: state.voteAggregate,
      pairs:     state.pairs,
      timestamp: Date.now(),
      round:     rnd,
    });
